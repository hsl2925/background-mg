...
exports.getShopOrderInfoData = (req, res) => {
  const sql = `select * from out_record_form order by id asc`
...
// 新增出库记录
exports.addOutProductRecord = (req, res) => {
  // 定义查重的 sql 语句
  const sql = `select * from out_record_form where del_note_number=? or item_number=? order by id desc`

  let datas = [req.body.del_note_number, req.body.item_number]
  db.query(sql, datas, (error, results) => {
    if (error) return res.cc(error)
    if (results.length === 2) return res.cc('出库单号/商品编号已存在 请重新输入!')
    if (results.length === 1 && results[0].del_note_number === req.body.del_note_number && results[0].item_number === req.body.item_number)
    if (results.length === 1 && results[0].del_note_number === req.body.del_note_number) return res.cc('出库单号已存在, 请重新输入!')
    if (results.length === 1 && results[0].item_number === req.body.item_number) return res.cc('商品编号已存在, 请重新输入!')

    // TODO: 新增出库记录数据
    const sql = `insert into out_record_form set?`
...
// 获取全部出库记录数据
exports.getOutProductRecordData = (req, res) => {
...
// 根据id 获取出库记录数据
exports.getOutProductRecordById = (req, res) => {
  const sql = `select * from purchase_order_form where id=?`
  db.query(sql, req.params.id, (err, results) => {
    if (err) return res.cc(err)

    if (results.length !==1) return res.cc('获取出库记录数据失败!')
...
// 更新出库记录信息
exports.updateOutProductRecordById = (req, res) => {
  ...
  db.query(sql, datas, (err, results) => {
    ...
    db.query(sql, [req.body, req.body.Id], (err, results) => {
      if (err) return res.cc(err)
      if (results.affectedRows !== 1) return res.cc('更新出库记录数据失败!')
...
// 删除出库记录数据的处理函数
exports.deleteOutProductRecordById = (req, res) => {
  const sql = `delete from purchase_order_form where id=?`
  db.query(sql, req.params.id, (err, results) => {
    if (err) return res.cc(err)
    if (results.affectedRows !==1) return res.cc('删除出库记录数据失败!')
    res.cc('删除出库记录数据成功!', 0)
  })
}

// 分页查询
exports.getOutProductRecordPage = (req, res) => {
...
// 出库记录列表查询条件接口
exports.outProductRecordSearch = (req, res) => {
