<template>
  ...
  <div class="main_index">
    ...
    <el-card>
      ...
      <el-form :inline="true" :model="queryInfo" ref="queryInfoRef" class="demo-form-inline">
        ...
        <el-form-item label="入库单号">
          <el-input v-model="queryInfo.de_note_number" placeholder="请输入入库单号"></el-input>
        </el-form-item>
        <el-form-item label="负责人">
          <el-input v-model="queryInfo.de_note_person" placeholder="请输入负责人"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        <el-table-column prop="de_note_number" label="入库单号" width="260">
        </el-table-column>
        <el-table-column prop="de_note_date" label="入库日期" width="230">
        </el-table-column>
        <el-table-column prop="de_note_person" label="负责人" width="260">
        </el-table-column>
        <el-table-column prop="item_number" label="商品编号" width="220">
        </el-table-column>
    ...
    <el-dialog title="新增商品入库"
      ...
      <el-form :model="addFormProduct" :rules="addSupplierRules"
       ref="addSupplierRef" label-width="100px">
        <el-form-item label="入库单号" prop="de_note_number">
          <el-input v-model="addFormProduct.de_note_number"></el-input>
        </el-form-item>
        <el-form-item label="入库日期" prop="de_note_date">
          <el-date-picker
            v-model="addFormProduct.de_note_date"
            type="date"
            :editable = true
        ...
        <el-form-item label="负责人" prop="de_note_person">
          <el-input v-model="addFormProduct.de_note_person"></el-input>
        </el-form-item>
    ...
    <!-- 修改数据 -->
    <el-dialog title="修改入库记录" :visible.sync="editDialogVisible" width="30%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules"
       ref="editFormRef" label-width="100px">
        <el-form-item label="入库单号">
          <el-input v-model="editForm.de_note_number"></el-input>
        </el-form-item>
        <el-form-item label="入库日期">
          <el-date-picker
            v-model="editForm.de_note_date"
            type="date"
            :editable = true
            value-format = "yyyy-MM-dd"
            placeholder="请选择入库日期">
        ...
        <el-form-item label="负责人">
          <el-input v-model="editForm.de_note_person"></el-input>
        </el-form-item>
        <el-form-item label="商品编号" prop="item_number">
    ...
    <!-- 外部按钮修商品入库数据 -->
    <el-dialog title="修改入库记录" :visible.sync="updateDialogVisible" width="30%">
      <el-form :model="editInfoBtb" :rules="editFormRules" ref="editFormRef" label-width="100px">
        <el-form-item label="入库单号">
          <el-input v-model="editInfoBtb.de_note_number"></el-input>
        </el-form-item>
        <el-form-item label="入库日期">
          <el-date-picker
            v-model="editInfoBtb.de_note_date"
            type="date"
            :editable = true
            value-format = "yyyy-MM-dd"
            placeholder="请选择入库日期">
        ...
        <el-form-item label="负责人">
          <el-input v-model="editInfoBtb.de_note_person"></el-input>
        </el-form-item>
        <el-form-item label="商品编号" prop="item_number">
...
export default {
  ...
  data () {
    return {
      ...
      addSupplierRules: {
        de_note_number: [{required: true, message: '请输入入库单号', trigger: 'blur'}],
  ...
  methods: {
    ...
    async getList (index) {
      ...
      const {data: res} = await this.$http.post('/tablesSearch', this.queryInfo)
      this.productList = res.data
      if (res.code !== 200) {
        this.$message.error('查询失败!!!')
      }
      if (this.queryInfo.item_number || this.queryInfo.de_note_number || this.queryInfo.de_note_person === '') {
        return 1
      }
    },
    // 搜索
    ...
    addInProduct () {
      this.$refs.addSupplierRef.validate(async valid => {
        if (!valid) return
        const {data: res} = await this.$http.post('/addTablesInfo/', this.addFormProduct)
        if (res.status !== 0) {
    ...
    async showEditDialog (id) {
      const {data: res} = await this.$http.get('/getTablesById/' + id)
      if (res.status !== 0) {
    ...
    editProductInfo () {
      this.$refs.editFormRef.validate(async valid => {
        ...
        const {data: res} = await this.$http.post('/updateTablesById/' + this.editForm.id, {
          Id: this.editForm.id,
          de_note_number: this.editForm.de_note_number,
          de_note_date: this.editForm.de_note_date,
          logistics_number: this.editForm.logistics_number,
          de_note_person: this.editForm.de_note_person,
    ...
    async deleteFormInfo (id) {
      ...
      const {data: res} = await this.$http.get('/deleteTablesById/' + id)
    ...
    async deleteFormData () {
      if (this.ids.length === 0) {
        ...
        const {data: res} = await this.$http.get('/deleteTablesById/' + this.ids)
    ...
    async showUpdateFromData () {
      if (this.ids.length === 0) {
        ...
        const {data: res} = await this.$http.get('/getTablesById/' + this.ids)
        if (res.status !== 0) {
          this.$message.error('查询数据失败!')
        }
    ...
    async updateProductInfo () {
      this.$refs.editFormRef.validate(async valid => {
        if (!valid) return // 表单预验证
        const {data: res} = await this.$http.post('/updateTablesById/' + this.editInfoBtb.id, {
          Id: this.editInfoBtb.id,
          de_note_number: this.editInfoBtb.de_note_number,
          de_note_date: this.editInfoBtb.de_note_date,
          logistics_number: this.editInfoBtb.logistics_number,
          de_note_person: this.editInfoBtb.de_note_person,
